...
PRO_DESC VARCHAR(60), CAT_ID INT NOT NULL, 
PRIMARY KEY (PRO_ID), 
...
(SELECT `order`.*, 
customer.cus_gender, 
customer.cus_name from `order` inner join customer where `order`.cus_id=customer.cus_id having `order`.ord_amount>=3000);

(select product.pro_name, 
`order`.* from `order`, 
supplier_pricing, product
...
product.pro_name from `order` inner join supplier_pricing on
